...
int main(int argc, const char **argv) {
  ...
  // Initialize the variables with the default values to make the development
  // more convenient
  float start_x = 10.f;
  float start_y = 10.f;
  float end_x = 90.f;
  ...
  // User input for the start and end points
  std::cout << "Please insert the start and end point cooridnates: "
            << std::endl;
  std::cout << "start_x = ";
  std::cin >> start_x;
  std::cout << "start_y = ";
  ...
  if ((start_x < 0.f) || (start_y < 0.f) || (end_x > 100.f) ||
      (end_y > 100.f)) {
    std::cerr << "Input was out of the range 0 - 100. Please insert a valid "
                 "input for all route planning parameters the next time.\n";
    exit(0);
  }
